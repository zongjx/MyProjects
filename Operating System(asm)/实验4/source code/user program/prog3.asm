...
;prog3
START:
    ;mov ax,0990h         ;决定程序内存地址的起始为0990h
	mov ax,cs
...
READKEY:
	...
	cmp al,'3'           ;比较al寄存器中的值是否与'1'相等，如果相等就把zf置为1，否则置为0
...
RETURN:
	...
	mov word[es:604h],0  ;把0存到604h的地址
...
DRMOVE:
	...
	jz DLMOVE            ;跳转指令
	cmp word[x],24       ;比较x与11，如果相同，则跳转到URMOVE（判断字符是否到达纵向下边界）
	jz URMOVE            ;跳转指令
	inc word[y]          ;y加1
	inc word[x]          ;x加1
	jmp SHOW             ;跳转到PRINT
DLMOVE:
	...
	jz DRMOVE            ;跳转指令
	cmp word[x],24       ;比较x与11，如果相同，则跳转到ULMOVE（判断字符是否到达纵向下边界）
	jz ULMOVE            ;跳转指令
	dec word[y]          ;y减1
	inc word[x]          ;x加1
	jmp SHOW             ;跳转到PRINT
URMOVE:
	...
	jz ULMOVE            ;跳转指令
	cmp word[x],12        ;比较x与12，如果相同，则跳转到DRMOVE（判断字符是否到达纵向上边界）
	jz DRMOVE            ;跳转指令
	inc word[y]          ;y加1
	dec word[x]          ;x减1
	jmp SHOW             ;跳转到PRINT
ULMOVE:
	...
	jz URMOVE            ;跳转指令
	cmp byte[x],12        ;比较x与12，如果相同，则跳转到DRMOVE（判断字符是否到达纵向上边界）
	jz DLMOVE            ;跳转指令
	dec word[y]          ;y减1
	dec word[x]          ;x减1
	jmp SHOW             ;跳转到PRINT
...
STEP1:
	...
	mov cx,22            ;把22传到cx寄存器中（共22列）
STEP2:
    call CAL             ;跳到CAL段
	mov byte[es:bx],' '  ;用空格填补多余位置
	mov byte[es:bx+1],1fh;蓝色底
	inc word[yy]         ;yy加1
	loop STEP2           ;跳到STEP2循环
	pop cx               ;弹出栈顶元素，存到cx中
	inc word[xx]         ;xx加1
	loop STEP1           ;跳到STEP1循环
	mov word[xx],23      ;把0传到xx中（下一行）
...
STEP3: 
	...
	mov word[xx],24      ;把1传到xx中（下一行）
    mov word[yy],7      ;把5传到yy中
	mov si,0             ;字符串计数器，初始化为0
	mov cx,8            ;字符串长度为8
STEP4: 
    call CAL             ;跳到CAL段
	...
	mov ax,0             ;把0存到ax寄存器中
	mov es,ax            ;把ax存进es寄存器中(初始地址为0)
	cmp word [es:604h],1 ;比较604h地址的值是否为1，如果是，则把zf置1，否则置0
	pop es               ;弹出es
...
DEFINE:                    ;宏定义
	...
	run db 0
	dir db d_l	
	name1 db 'coded by zhouchancheng'  ;周禅城
