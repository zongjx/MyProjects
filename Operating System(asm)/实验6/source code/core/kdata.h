
#define NEW 0
#define READY 1
#define RUNNING 2
#define EXIT 3
#define BLOCKED 4

int stacks[20000];
int *fstack=&stacks[2000];
typedef struct RegisterImage{
	int SS;
	int GS;
	int FS;
	int ES;
	int DS;
	int DI;
	int SI;
	int BP;
	int SP;
	int BX;
	int DX;
	int CX;
	int AX;
	int IP;
	int CS;
	int FLAGS;
}RegisterImage;

typedef struct PCB{
	RegisterImage regImg;
	int Process_Status;
	int f_pcbid;
	int pcbid;
}PCB;


PCB pcb_list[8];
int CurrentPCBno = 0; 
int Program_Num = 0;


extern void printChar();

PCB* Current_Process();
void Save_Process(int, int, int, int, int, int, int, int,
		  int,int,int,int, int,int, int,int );
void init(PCB*, int, int,int);
void Schedule();
void special();



void Save_Process(int gs,int fs,int es,int ds,int di,int si,int bp,
		int sp,int dx,int cx,int bx,int ax,int ss,int ip,int cs,int flags)
{
	pcb_list[CurrentPCBno].regImg.AX = ax;
	pcb_list[CurrentPCBno].regImg.BX = bx;
	pcb_list[CurrentPCBno].regImg.CX = cx;
	pcb_list[CurrentPCBno].regImg.DX = dx;

	pcb_list[CurrentPCBno].regImg.DS = ds;
	pcb_list[CurrentPCBno].regImg.ES = es;
	pcb_list[CurrentPCBno].regImg.FS = fs;
	pcb_list[CurrentPCBno].regImg.GS = gs;
	pcb_list[CurrentPCBno].regImg.SS = ss;

	pcb_list[CurrentPCBno].regImg.IP = ip;
	pcb_list[CurrentPCBno].regImg.CS = cs;
	pcb_list[CurrentPCBno].regImg.FLAGS = flags;
	
	pcb_list[CurrentPCBno].regImg.DI = di;
	pcb_list[CurrentPCBno].regImg.SI = si;
	pcb_list[CurrentPCBno].regImg.SP = sp;
	pcb_list[CurrentPCBno].regImg.BP = bp;
}
int y;
void Schedule(){
	
    if(pcb_list[CurrentPCBno].Process_Status == RUNNING)
	pcb_list[CurrentPCBno].Process_Status = READY;

	CurrentPCBno ++;
	if( CurrentPCBno > Program_Num )
		CurrentPCBno = 1;
	while(pcb_list[CurrentPCBno].Process_Status == BLOCKED || pcb_list[CurrentPCBno].Process_Status == EXIT) {
		CurrentPCBno ++;
	}

	if( pcb_list[CurrentPCBno].Process_Status != NEW )
		pcb_list[CurrentPCBno].Process_Status = RUNNING; 
}

PCB* Current_Process(){

	return &pcb_list[CurrentPCBno];
}

void init(PCB* pcb,int segement, int offset,int num)
{
	pcb->regImg.GS = 0xb800;
	pcb->regImg.SS = segement;
	pcb->regImg.ES = segement;
	pcb->regImg.DS = segement;
	pcb->regImg.CS = segement;
	pcb->regImg.FS = segement;
	pcb->regImg.IP = offset;
    
	pcb->regImg.AX = 0;
	pcb->regImg.BX = 0;
	pcb->regImg.CX = 0;
	pcb->regImg.DX = 0;
	pcb->regImg.DI = 0;
	pcb->regImg.SI = 0;
	pcb->regImg.BP = 0;
	pcb->regImg.FLAGS = 512;
	pcb->Process_Status = NEW;
}

void special()
{
	if(pcb_list[CurrentPCBno].Process_Status==NEW)
		pcb_list[CurrentPCBno].Process_Status=RUNNING;
}

/*int do_fork(){
	PCB* p;
	p = pcb_list + Program_Num + 1;
	if( p >= pcb_list + 8 )
		pcb_list[CurrentPCBno].regImg.AX = -1;
	else
	{
	pcb_list[p].regImg.SS = Segment;
	pcb_list[p].regImg.SP = pcb_list[CurrentPCBno].regImg.SP;
	pcb_list[p].regImg.ES = pcb_list[CurrentPCBno].regImg.ES;
	pcb_list[p].regImg.DS = pcb_list[CurrentPCBno].regImg.DS;
	pcb_list[p].regImg.CS = pcb_list[CurrentPCBno].regImg.CS;
	pcb_list[p].regImg.IP = pcb_list[CurrentPCBno].regImg.IP;
	pcb_list[p].regImg.AX = pcb_list[CurrentPCBno].regImg.AX;
	pcb_list[p].regImg.BX = pcb_list[CurrentPCBno].regImg.BX;
	pcb_list[p].regImg.CX = pcb_list[CurrentPCBno].regImg.CX;
	pcb_list[p].regImg.DX = pcb_list[CurrentPCBno].regImg.DX;
	pcb_list[p].regImg.DI = pcb_list[CurrentPCBno].regImg.DI;
	pcb_list[p].regImg.SI = pcb_list[CurrentPCBno].regImg.SI;
	pcb_list[p].regImg.BP = pcb_list[CurrentPCBno].regImg.BP;
	pcb_list[p].regImg.FLAGS = pcb_list[CurrentPCBno].regImg.FLAGS;
	pcb_list[p].Process_Status = EXIT;
	pcb_list[p].pcbid = p;
	pcb_list[p].f_pcbid = CurrentPCBno;
    memcpy((void*)pcb_list[p].regImg.SS,(void*)pcb_list[CurrentPCBno].regImg.SS,0x100);
	pcb_list[CurrentPCBno].regImg.AX = pcb_list[p].pcbid;
	pcb_list[p].regImg.AX = 0;
	pcb_list[p].Process_Status = READY;
	return pcb_list[CurrentPCBno].regImg.AX;
	}
}*/

void memcopy( PCB* F_PCB, PCB* C_PCB )               /*复制寄存器*/
{
	/*C_PCB -> regImg.SP = F_PCB -> regImg.SP;*/
	C_PCB -> regImg.SS = F_PCB -> regImg.SS;
	C_PCB -> regImg.GS = F_PCB -> regImg.GS;
	C_PCB -> regImg.ES = F_PCB -> regImg.ES;
	C_PCB -> regImg.DS = F_PCB -> regImg.DS;
	C_PCB -> regImg.CS = F_PCB -> regImg.CS;
	C_PCB -> regImg.FS = F_PCB -> regImg.FS;
	C_PCB -> regImg.IP = F_PCB -> regImg.IP;
	C_PCB -> regImg.AX = F_PCB -> regImg.AX;
	C_PCB -> regImg.BX = F_PCB -> regImg.BX;
	C_PCB -> regImg.CX = F_PCB -> regImg.CX;
	C_PCB -> regImg.DX = F_PCB -> regImg.DX;
	C_PCB -> regImg.DI = F_PCB -> regImg.DI;
	C_PCB -> regImg.SI = F_PCB -> regImg.SI;
	C_PCB -> regImg.BP = F_PCB -> regImg.BP;
	C_PCB -> regImg.FLAGS = F_PCB -> regImg.FLAGS;
}

/*int do_fork()
{
	int i;
	PCB* p;
	p = pcb_list + Program_Num + 1;
	if( p > pcb_list + 8 )
		pcb_list[CurrentPCBno].regImg.AX = -1;
	else
	{
		Program_Num++;
		memcopy( pcb_list + CurrentPCBno, p );
        /*memcpy( pcb_list[CurrentPCBno].regImg.SS, p ->regImg.SS , 0x100 );*//*
		p->regImg.SP = pcb_list[CurrentPCBno].regImg.SP + 4000;
		for(i=0;i<2000;i++){
			stacks[4000-i]=stacks[2000-i];
		}
		p -> pcbid = p - pcb_list;
		p -> f_pcbid = CurrentPCBno;
		p -> Process_Status = READY;
		pcb_list[CurrentPCBno].regImg.AX = p -> pcbid;
		p -> regImg.AX = 0;
		/*PCB_Restart();*//*
		return pcb_list[CurrentPCBno].regImg.AX;
	}
}*/

/*int do_fork()
{
	int i;
	int p;
	for(p = 0;p < 8;p++)
	{
		if(pcb_list[CurrentPCBno].Process_Status == NEW ) break;
	}
	if( p > 8 )
		pcb_list[CurrentPCBno].regImg.AX = -1;
	else
	{
		Program_Num++;
		pcb_list[p].regImg.SS = pcb_list[CurrentPCBno].regImg.SS;
		pcb_list[p].regImg.ES = pcb_list[CurrentPCBno].regImg.ES;
		pcb_list[p].regImg.DS = pcb_list[CurrentPCBno].regImg.DS;
		pcb_list[p].regImg.CS = pcb_list[CurrentPCBno].regImg.CS;
		pcb_list[p].regImg.IP = pcb_list[CurrentPCBno].regImg.IP;
		pcb_list[p].regImg.AX = pcb_list[CurrentPCBno].regImg.AX;
		pcb_list[p].regImg.BX = pcb_list[CurrentPCBno].regImg.BX;
		pcb_list[p].regImg.CX = pcb_list[CurrentPCBno].regImg.CX;
		pcb_list[p].regImg.DX = pcb_list[CurrentPCBno].regImg.DX;
		pcb_list[p].regImg.DI = pcb_list[CurrentPCBno].regImg.DI;
		pcb_list[p].regImg.SI = pcb_list[CurrentPCBno].regImg.SI;
		pcb_list[p].regImg.BP = pcb_list[CurrentPCBno].regImg.BP;
		pcb_list[p].regImg.FLAGS = pcb_list[CurrentPCBno].regImg.FLAGS;
		/*memcopy( pcb_list + CurrentPCBno, p );
        /*memcpy( pcb_list[CurrentPCBno].regImg.SS, p ->regImg.SS , 0x100 );*//*
		pcb_list[p].regImg.SP = pcb_list[CurrentPCBno].regImg.SP + 4000;
		for(i=0;i<2000;i++){
			stacks[4000-i]=stacks[2000-i];
		}
		pcb_list[p].pcbid = p;
		pcb_list[p].f_pcbid = CurrentPCBno;
		pcb_list[p].Process_Status = READY;
		pcb_list[CurrentPCBno].regImg.AX = pcb_list[p].pcbid;
		pcb_list[p].regImg.AX = 0;
		/*PCB_Restart();*//*
		return pcb_list[CurrentPCBno].regImg.AX;
	}
}*/


int do_fork()
{
	if(CurrentPCBno != 1)  
	{return 0;}
else{
	int i;
	PCB* p;
	p=&pcb_list[1];
	while(p<=&pcb_list[7]&&p->Process_Status!=NEW) {p++;}            /*寻找空的pcb块*/
	
	if( p > &pcb_list[7] )
		pcb_list[CurrentPCBno].regImg.AX = -1;                       /*超出范围，返回-1*/
	else
	{
		Program_Num++;
		memcopy( pcb_list + CurrentPCBno, p );                       /*复制*/
        /*memcpy( pcb_list[CurrentPCBno].regImg.SS, p ->regImg.SS , 0x100 );*/
		
		for(i=0;i<2000;i++){
			stacks[4000-i]=stacks[2000-i];                           /*堆栈的复制*/
		}
		p->regImg.SP = pcb_list[CurrentPCBno].regImg.SP + 4000;      /*改变sp的值*/
		p->pcbid = p - pcb_list;
		p->f_pcbid = CurrentPCBno;
		p->Process_Status = READY;
		pcb_list[CurrentPCBno].regImg.AX = p->pcbid;
		p->regImg.AX = 0;
		return pcb_list[CurrentPCBno].regImg.AX;                     
	}
}
}


int do_wait() {
   pcb_list[CurrentPCBno].Process_Status = BLOCKED;                  /*阻塞父进程*/
   Schedule();                                                       /*轮转到子进程*/
   /*PCB_Restart();*/
   return pcb_list[CurrentPCBno].regImg.AX;                         
}

int do_exit(char ch) {
	pcb_list[CurrentPCBno].Process_Status = EXIT;
    pcb_list[pcb_list[CurrentPCBno].f_pcbid].Process_Status = READY;
    pcb_list[pcb_list[CurrentPCBno].f_pcbid].regImg.AX=ch;
    Schedule();
	/*PCB_Restart();*/
	return CurrentPCBno;
}
