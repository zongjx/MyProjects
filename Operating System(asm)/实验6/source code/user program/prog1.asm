...
NOTREAD:
	...
	cmp word[count],0affh ;把count与0afh作比较，如果相等，则把zf置为1，否则为0(程序运行时间)
...
PRINT:
	...
	jmp BEGIN            ;跳转到BEGIN
SHOW_NAME:             
    mov word[xx],1      ;把0传到xx中（显示姓名学号的纵坐标）
	mov cx,2             ;把2传到cx寄存器中（共2行）
STEP1:
    push cx              ;把cx压栈
	mov word[yy],1      ;把0传到yy中（显示姓名学号的横坐标）
...
STEP2:
	...
	mov word[xx],1      ;把0传到xx中（下一行）
	mov word[yy],1      ;把0传到yy中
