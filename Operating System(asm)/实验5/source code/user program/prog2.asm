...
;prog2
START:
    ;mov ax,0950h         ;决定程序内存地址的起始为0950h
	mov ax,cs
...
READKEY:
	...
	cmp al,'2'           ;比较al寄存器中的值是否与'1'相等，如果相等就把zf置为1，否则置为0
...
RETURN:
	...
	mov word[es:602h],0  ;把0存到602h的地址
...
URMOVE:
	...
	cmp word[y],79       ;比较y与38，如果相同，则跳转到ULMOVE（判断字符是否到达横向右边界）
	jz ULMOVE            ;跳转指令
...
STEP1:
    push cx              ;把cx压栈
	mov word[yy],58      ;把0传到yy中（显示姓名学号的横坐标）
	mov cx,22            ;把22传到cx寄存器中（共22列）
STEP2:
    call CAL             ;跳到CAL段
	mov byte[es:bx],' '  ;用空格填补多余位置
	mov byte[es:bx+1],0fh;黑色底
	inc word[yy]         ;yy加1
...
STEP4: 
	...
	mov ax,0             ;把0存到ax寄存器中
	mov es,ax            ;把ax存进es寄存器中(初始地址为0)
	cmp word [es:602h],1 ;比较602h地址的值是否为1，如果是，则把zf置1，否则置0
	pop es               ;弹出es
...
DEFINE:                    ;宏定义
	...
	run db 0
	dir db d_l	
	name1 db 'coded by zhonglingshan'  ;钟凌山
	name2 db '15352443' ;15352443
	times 1022-($-$$) db 0
	db 0x55,0xaa